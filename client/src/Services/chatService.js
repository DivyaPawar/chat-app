...
import { authHeader, authHeaderImg } from '../Utilities/auth-header';
...
export function useSendCoversationImage(){
    const { enqueueSnackbar } = useSnackbar();
    const handleResponse = useHandleResponse();

    const sendCoversationImage = (id, file) => {
        const requestOptions = {
            method: 'POST',
            headers: authHeaderImg(),
            body: JSON.stringify({ to: id, file: file }),
        };
 console.log(requestOptions);
        return fetch(
            `${process.env.REACT_APP_API_URL}/api/messages/uploadfiles/`,
            requestOptions
        )
